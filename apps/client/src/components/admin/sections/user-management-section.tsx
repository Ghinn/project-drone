...
export default function UserManagementSection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (currentUser) {
        ...
        setNotification({ 
          type: 'success', 
          message: 'Pengguna baru berhasil ditambahkan. Tautan pembuatan sandi telah dikirim!' 
        });
      }
    ...
    try {
      ...
      setNotification({ type: 'success', message: `Status berhasil diubah menjadi ${newStatus === 'APPROVED' ? 'ACTIVE' : 'INACTIVE'}` });
    } catch (error: any) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setNotification({ type: 'success', message: 'Pengguna berhasil dihapus permanen.' });
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[#191919] dark:text-white">Manajemen Pengguna</h1>
          <p className="text-sm text-[#5B6068] mt-0.5">Kelola akun dan otorisasi hak akses platform pemantauan.</p>
        </div>
      </div>
      ...
      {notification && (
        <div className={`p-4 rounded-xl border flex items-center justify-between text-sm font-semibold
          ...
          <button onClick={() => setNotification(null)} className="text-xs underline font-normal">Tutup</button>
        </div>
      )}

      {/* STATS CARDS (Mengacu pada Gambar Pengguna.png) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-[#16161a] p-5 rounded-xl border border-[#E5E7EB] dark:border-zinc-800 shadow-sm">
          <p className="text-sm font-semibold text-[#191919] dark:text-zinc-300">Semua Pengguna</p>
          <div className="mt-2 flex items-baseline gap-1.5">
            <span className="text-3xl font-bold text-[#191919] dark:text-white">{stats.total}</span>
            <span className="text-xs font-medium text-[#84994F]">akun</span>
          </div>
          <p className="text-xs text-[#6A717F] mt-1">Total terdaftar di sistem</p>
        </div>

        <div className="bg-white dark:bg-[#16161a] p-5 rounded-xl border border-[#E5E7EB] dark:border-zinc-800 shadow-sm">
          <p className="text-sm font-semibold text-[#191919] dark:text-zinc-300">Farmer</p>
          <div className="mt-2 flex items-baseline gap-1.5">
            <span className="text-3xl font-bold text-[#191919] dark:text-white">{stats.farmer}</span>
            <span className="text-xs font-medium text-[#84994F]">akun</span>
          </div>
        ...
        <div className="bg-white dark:bg-[#16161a] p-5 rounded-xl border border-[#E5E7EB] dark:border-zinc-800 shadow-sm">
          <p className="text-sm font-semibold text-[#191919] dark:text-zinc-300">Operator</p>
          <div className="mt-2 flex items-baseline gap-1.5">
            <span className="text-3xl font-bold text-[#191919] dark:text-white">{stats.operator}</span>
            <span className="text-xs font-medium text-[#84994F]">akun</span>
          </div>
        ...
        <div className="bg-white dark:bg-[#16161a] p-5 rounded-xl border border-[#E5E7EB] dark:border-zinc-800 shadow-sm">
          <p className="text-sm font-semibold text-[#191919] dark:text-zinc-300">Admin</p>
          <div className="mt-2 flex items-baseline gap-1.5">
            <span className="text-3xl font-bold text-[#191919] dark:text-white">{stats.admin}</span>
            <span className="text-xs font-medium text-[#84994F]">akun</span>
          </div>
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          <div className="relative flex-1 md:w-64">
            ...
            <input 
              ...
              className="w-full pl-9 pr-4 py-2 border border-[#E5E7EB] dark:border-zinc-800 rounded-xl bg-white dark:bg-[#16161a] text-sm text-[#191919] dark:text-white outline-none focus:border-[#84994F] transition-all"
            />
          </div>

          <button
            type="button"
            className="p-2.5 border border-[#E5E7EB] dark:border-zinc-800 rounded-xl bg-white dark:bg-[#16161a] text-[#5B6068] hover:text-[#191919] transition-colors"
            title="Filter data"
          >
          ...
          <button 
            ...
            className="inline-flex items-center gap-2 px-4 py-2.5 bg-[#84994F] hover:bg-[#e65c00] text-white text-sm font-semibold rounded-xl shadow-sm transition-colors whitespace-nowrap"
          >
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#84994F] text-white">
                <th className="px-6 py-3.5 text-xs font-bold uppercase tracking-wider">No.</th>
                <th className="px-6 py-3.5 text-xs font-bold uppercase tracking-wider">Nama Pengguna</th>
                <th className="px-6 py-3.5 text-xs font-bold uppercase tracking-wider">Email Pengguna</th>
                <th className="px-6 py-3.5 text-xs font-bold uppercase tracking-wider">Tanggal Bergabung</th>
                <th className="px-6 py-3.5 text-xs font-bold uppercase tracking-wider">Hak Akses</th>
            ...
            <tbody className="divide-y divide-[#E5E7EB] dark:divide-zinc-800/60">
              {isLoading ? (
                <tr>
                  <td colSpan={7} className="px-6 py-12 text-center text-sm text-[#6A717F]">
                ...
                paginatedUsers.map((user, index) => {
                  ...
                  return (
                    <tr key={user.id} className="hover:bg-gray-50/60 dark:hover:bg-zinc-800/20 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm font-mono text-[#6A717F]">#{shortId}</td>
                      <td className="px-6 py-4 text-sm font-bold text-[#191919] dark:text-white">{user.name || 'Tanpa Nama'}</td>
                      <td className="px-6 py-4 text-sm text-[#5B6068] dark:text-zinc-400">{joinDate}</td>
                      <td className="px-6 py-4 text-sm text-[#5B6068] dark:text-zinc-400">{user.email}</td>
                        ...
                        <span className="inline-flex items-center gap-1.5 text-sm font-semibold text-[#191919] dark:text-zinc-200">
                          <span className={`w-2 h-2 rounded-full 
                            ${user.role === 'ADMIN' ? 'bg-red-500' : 
                              ...
                              user.role === 'FARMER' ? 'bg-emerald-500' : 'bg-gray-400'}`} />
                          {user.role}
                        </span>
                      </td>
                ...
                <tr>
                  <td colSpan={7} className="px-6 py-10 text-center text-sm text-[#6A717F]">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-[#16161a] border border-[#E5E7EB] dark:border-zinc-800 w-full max-w-md p-6 rounded-2xl shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-[#6A717F] mb-1.5">Nama Lengkap</label>
                <input 
                  type="text" 
                  name="name"
                  value={formData.name}
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-[#6A717F] mb-1.5">Alamat Email</label>
                <input 
                  type="email" 
                  name="email"
              ...
              <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-[#6A717F] mb-1.5">Role Akses</label>
                  <select 
                    name="role"
                    value={formData.role}
              ...
              {!currentUser && (
                <div className="p-3 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/50 rounded-xl flex items-start gap-2.5 text-xs text-amber-800 dark:text-amber-300">
                  ...
                  <span>Pengguna akan menerima pesan di email mereka untuk mengatur kata sandi sebelum bisa login ke dalam platform.</span>
                </div>
              )}
              ...
              <div className="pt-4 flex justify-end gap-3 border-t border-[#E5E7EB] dark:border-zinc-800 mt-6">
                ...
                >
                  {isSubmitting ? 'Menyimpan...' : currentUser ? 'Simpan Perubahan' : 'Kirim Undangan'}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-[#16161a] border border-[#E5E7EB] dark:border-zinc-800 w-full max-w-sm p-6 rounded-2xl shadow-2xl animate-in zoom-in-95 duration-200 text-left">
            ...
            <p className="text-sm text-[#5B6068] dark:text-zinc-400 mb-6 leading-relaxed">
              Apakah Anda yakin ingin menghapus akun <span className="font-semibold text-[#191919] dark:text-white">{currentUser.name || currentUser.email}</span>? Tindakan ini akan menghapus aksesnya secara permanen dari sistem.
            </p>
