...
import { ClipboardClock, Drone, LayoutDashboard, Menu, Settings, X, SatelliteDish, ArrowLeft } from 'lucide-react';
...
const NAV_ICONS: Record<string, React.ReactNode> = {
  ...
  telemetry: <SatelliteDish size={16} />,
  drone: <Drone size={16} />,
  listClock: <ClipboardClock size={16} />,
  settings: <Settings size={16} />,
};

export default function MonitoringOperatorNavbar() {
  const router = useRouter();
  const { activeTab, setActiveTab, isSidebarOpen, setIsSidebarOpen, navItems } = useMonitoringOperator();
      ...
      >
        ...
        <div className="flex items-center gap-3 px-3.5 h-14 shrink-0 border-b border-gray-200 dark:border-[#1e1e1e]">
          ...
          <div className={`flex justify-between items-center w-full ${!isSidebarOpen ? 'hidden md:hidden' : 'flex'}`}>
            ...
            {/* button close sidebar di desktop */}
            <button
              onClick={() => setIsSidebarOpen(false)}
          ...
          {navItems.map((n) => {
            ...
            return (
              <button
                ...
                title={!isSidebarOpen ? `${n.label} · ${n.labelEn}` : undefined}
