...
exports.getTelemetryLogStateByDrone = exports.telemetryEmitter = void 0;
exports.initMqtt = initMqtt;
const mqtt_1 = __importDefault(require("mqtt"));
const prisma_js_1 = require("../lib/prisma.js");
const events_1 = require("events");
exports.telemetryEmitter = new events_1.EventEmitter();
const activeDronesState = new Map();
function initMqtt() {
    const client = mqtt_1.default.connect('mqtt://mqtt.dreampalm.id:1883', {
    ...
    client.on('message', async (topic, message) => {
        try {
            ...
            if (topicType === 'status') {
                ...
                try {
                    await prisma_js_1.prisma.drone.upsert({
                        where: { id: droneId },
                        update: { status: statusStr },
                        create: { id: droneId, status: statusStr }
                    });
                    try {
                        exports.telemetryEmitter.emit(`status_update_${droneId}`, { status: statusStr });
                        exports.telemetryEmitter.emit('status_update_all', { droneId: droneId, status: statusStr });
                    }
                    catch (emitError) {
            ...
            if (topicType === 'telemetryState') {
                ...
                try {
                    exports.telemetryEmitter.emit(`telemetry_update_${droneId}`, updatedState);
                }
                catch (emitError) {
                    console.error(`[MQTT] Emitter Error pada Telemetri ${droneId}:`, emitError);
                }
                try {
                    await prisma_js_1.prisma.telemetryLog.create({
                        data: {
                            droneId: droneId,
...
const getTelemetryLogStateByDrone = (droneId) => {
    return activeDronesState.get(droneId) || null;
};
exports.getTelemetryLogStateByDrone = getTelemetryLogStateByDrone;
